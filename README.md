...
基于 **DeepAgents SDK**（LangChain/LangGraph）构建的生产级多 Agent 项目脚手架。
...
- **基础设施** — YAML 配置（热重载）、模型工厂、结构化日志、IM 通道
...
本脚手架支持 DeepAgents `AgentMiddleware`：

| 中间件 | 用途 |
|---|---|
| `MemoryMiddleware` | 将记忆上下文注入系统提示词 |
| `SkillsMiddleware` | 加载并注入技能指令 |
| `SubAgentMiddleware` | 启用子 Agent 委派 |
| `FilesystemMiddleware` | 文件操作工具 |
| `LoopDetectionMiddleware` | 检测并中断重复性工具调用循环 |
| `ToolErrorHandlingMiddleware` | 捕获工具异常并转换为错误消息 |
| `DynamicContextMiddleware` | 将日期/时间与记忆注入上下文 |
| `TokenUsageMiddleware` | 逐轮追踪并记录 Token 消耗 |
| `SafetyTerminationMiddleware` | 检测提供商的安全拒绝信号 |
| `TodoMiddleware` | 管理待办列表并检测上下文丢失 |
