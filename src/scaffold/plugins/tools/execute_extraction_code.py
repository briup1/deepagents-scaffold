...
async def execute_extraction_code(task_id: str) -> dict[str, Any]:
    ...
    script_artifact = await _get_artifact(task.script_artifact_id)
    if script_artifact is None:
        return {"error": f"脚本工件 {task.script_artifact_id} 不存在"}

    upload_artifact = await _get_artifact(task.upload_artifact_id)
    if upload_artifact is None:
        return {"error": f"上传工件 {task.upload_artifact_id} 不存在"}

    script_bytes = await _read_artifact_bytes(script_artifact)
    upload_bytes = await _read_artifact_bytes(upload_artifact)
    ...
    extraction_artifact = await _save_artifact(
