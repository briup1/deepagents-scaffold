...
class ModelRetryAdapter(AgentMiddleware):
    ...
    async def awrap_model_call(self, request: Any, handler: Any) -> Any:
        return await self._middleware.awrap_model_call(request, self._wrap_async_handler(request, handler))

    def _wrap_sync_handler(self, request: Any, handler: Any) -> Any:
        thread_id = _extract_thread_id(request)
        model = _extract_model_name(request)
        state = {"attempt": 0}
        ...
        def wrapped(req: Any) -> Any:
            ...
            try:
                ...
                logger.warning(
                    "model call attempt %d failed: thread_id=%s model=%s status_code=%s exc=%s",
                    state["attempt"],
                    thread_id,
                    model,
                    status_code,
            ...
            if state["attempt"] > 1:
                latency_ms = round((time.monotonic() - started) * 1000, 1)
                logger.info(
                    "model call recovered on attempt %d: thread_id=%s model=%s",
                    state["attempt"],
                    thread_id,
                    model,
                    extra={
